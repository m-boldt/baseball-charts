...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getPreviousGames({ state }) {
      axios
        .get(
          `https://hr-freeware.info/${
            state.tableName
          }?Active=false&_sort=createdAt:DESC`,
          {
            headers: {
              Authorization:
                'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI1YzRhMWJkOWMzMzdmNzMyYWQxMzNiOTIiLCJpZCI6IjVjNGExYmQ5YzMzN2Y3MzJhZDEzM2I5MiIsImlhdCI6MTU1NTg0NzU1MywiZXhwIjoxNTU4NDM5NTUzfQ.p0Vko9PEKgNVjAPxkzgLRl8-xEz0HiJ7ld8FuEuh-QQ'
    ...
    getActiveGames({ state }) {
      axios
        .get(
          `https://hr-freeware.info/${
            state.tableName
          }?Active=true&_sort=createdAt:DESC`,
          {
            headers: {
              Authorization:
                'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI1YzRhMWJkOWMzMzdmNzMyYWQxMzNiOTIiLCJpZCI6IjVjNGExYmQ5YzMzN2Y3MzJhZDEzM2I5MiIsImlhdCI6MTU1NTg0NzU1MywiZXhwIjoxNTU4NDM5NTUzfQ.p0Vko9PEKgNVjAPxkzgLRl8-xEz0HiJ7ld8FuEuh-QQ'
        ...
        .then(response => {
          if (response.data.length > 0) {
            state.activeGames = response.data.map(function(game) {
              return {
                id: game.id,
                date: game.createdAt,
                opponent: game.gameState.opponent,
    ...
    setGameState({ state }) {
      axios
        .get(
          `https://hr-freeware.info/${state.tableName}?id=${
            state.gameId
          }&Active=true&_limit=1`,
          {
            headers: {
              Authorization:
                'Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJfaWQiOiI1YzRhMWJkOWMzMzdmNzMyYWQxMzNiOTIiLCJpZCI6IjVjNGExYmQ5YzMzN2Y3MzJhZDEzM2I5MiIsImlhdCI6MTU1NTg0NzU1MywiZXhwIjoxNTU4NDM5NTUzfQ.p0Vko9PEKgNVjAPxkzgLRl8-xEz0HiJ7ld8FuEuh-QQ'
    ...
    setGameId({ state, dispatch }, gameId) {
      state.gameId = gameId;
